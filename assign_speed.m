...
for j = 2:size(in, 1)
if out(j,const.COL_SLOPE)-out(j-1,const.COL_SLOPE) < 0 
    if abs(out(j,const.COL_SLOPE))>abs(out(j-1,const.COL_SLOPE))
       out(j,const.COL_SPEED) = out(j-1,const.COL_SPEED)+((out(j-1,const.COL_SPEED)*abs(out(j,const.COL_SLOPE)))/100);  
    elseif abs(out(j,const.COL_SLOPE))<abs(out(j-1,const.COL_SLOPE))
       out(j,const.COL_SPEED) = out(j-1,const.COL_SPEED)-((out(j-1,const.COL_SPEED)*abs(out(j,const.COL_SLOPE)))/100); 
    end     
else out(j,const.COL_SLOPE)-out(j-1,const.COL_SLOPE) >= 0
    out(j,const.COL_SPEED) = out(j-1,const.COL_SPEED)-((out(j-1,const.COL_SPEED)*abs(out(j,const.COL_SLOPE)))/100);
end            
end
end
